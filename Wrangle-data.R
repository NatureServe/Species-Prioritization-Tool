...
# max.length <- 999
# x <- 1
# y <- min(c(max.length,length(ids)))
# dat<-dim(0)
# 
# for (j in 1:ceiling((length(ids)/max.length))) {
#   ids.temp <- ids[x:y]
#   
#   qry <- paste0("SELECT
...
# and egt.element_global_id IN (", paste0(ids.temp, collapse = ", "),")
# 
# ORDER BY 
...
#   x <- y +1
#   y <- min(c(x-1+max.length,length(ids)))
# }

## Alternatively, select all records from biotics that have a BLM_SSS_States entry
qry <- "SELECT
 egt.element_global_id
 , 'ELEMENT_GLOBAL.'||egt.element_global_ou_uid||'.'||egt.element_global_seq_uid eguid
 , GETNAMECATDESC(egt.gname_id) Name_Category
...
ja2023<- read.csv("C:/Users/max_tarjan/OneDrive - NatureServe/Documents/Partners-In-Conservation/Output-2023-01-09/blm_eo_jurisdiction_results-2023-01-09.csv") %>% 
