...
#' # Load Data
#' ## Initial scores
latest_scores <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1KIpQPLvHiJY1KvbGY3P04HwU2WESqKOQZYECpN_dxgo/edit?usp=sharing", sheet="ESA_spp_20221205") %>%
...
shinyApp(
      ...
      div(id = "user_interface",
          ...
          fluidRow(style = "padding-left: 50px; padding-top: 10px;",
                   ...
                   fluidRow(
                     ...
                     column(3, style = "padding-left: 30; margin-top: 30px;", prettySwitch(inputId = "reviewed_all", label = "Mark all scores as reviewed", status = "primary"))
                   ),
  ...
  server = function(input, output, session) {
    ...
    output$scores_table <- renderDT({
      ...
      datatable(state_scores$values, options = list(dom = 'tp', pageLength = 10), editable = list(target = "cell", disable = list(columns = c(1:12)), numeric = c(13:15), area = 16), selection = list(mode = "multiple", target = "row"))
    ...
    output$filtered_table <- renderDT({
      ...
      datatable(latest_scores_edits$values, options = list(dom = 'tp', pageLength = 10), editable = list(target = "cell", disable = list(columns = c(1:12)), numeric = c(13:15), area = 16), selection = list(mode = "multiple", target = "row"))
